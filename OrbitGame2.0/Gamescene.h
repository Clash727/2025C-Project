...
#include "obstacleitem.h"    // <<< 包含 obstacleitem.h 的完整定义
...
private slots:
    ...
    void handleObstacleHit(ObstacleItem* item); // 处理障碍物被撞击的槽
...
private:
    ...
    QList<ObstacleItem*> m_obstacles;       // 存储所有障碍物对象的列表

    // 辅助函数
    bool loadTracksAndCollectiblesFromFile(const QString& filename); // 函数名可考虑改为 loadLevelDataFromFile
